...
private:
  ...
  std::vector<int> head_prompt{1, 1, 32010};
...
void Phi3::load_sentencepiece(std::string tokenizer_path) {
  ...
  // EOS = sentencepiece.eos_id(); // TODO
  EOS = 32007;
  printf("Done!\n");
}

void Phi3::init(std::string model_path, std::string tokenizer_path) {
...
int Phi3::forward_first(std::vector<int> &tokens) {
...
int Phi3::forward_next(int cur_token) {
...
void Phi3::answer(const std::string &input_str) {
  int tok_num = 1;
  std::vector<int> tokens;
  std::vector<int> prompt{32010};
  std::vector<int> endwords{32007, 32001};
...
int main(int argc, char **argv) {
  ...
  printf("Demo for Phi3 in BM1688\n");
  ...
  Phi3.init(model_path, tokenizer_path);
  printf("==========================\n");
  Phi3.chat();
  Phi3.deinit();
  return 0;
