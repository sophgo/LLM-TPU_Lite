...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids=None, unsqueeze_dim=1):
    ...
    """
    cos = cos.unsqueeze(unsqueeze_dim).transpose(1, 2)
    sin = sin.unsqueeze(unsqueeze_dim).transpose(1, 2)
...
class Phi3MLP(nn.Module):
    ...
    def forward(self, hidden_states: torch.FloatTensor) -> torch.FloatTensor:
        ...
        gate = up_states[:, :, :up_states.shape[-1]//2]
        up_states = up_states[:, :, up_states.shape[-1]//2:]
...
class Phi3Attention(nn.Module):
    ...
    def forward(
        ...
        # print(value_states.shape, sin.shape)
