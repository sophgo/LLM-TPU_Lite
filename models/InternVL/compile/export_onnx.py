...
class VisionTransformer(torch.nn.Module):
    def __init__(self):
        ...
        self.mlp1 = nn.Sequential(
            ...
            nn.Linear(VIT_HIDDEN_SIZE * int(1 / DOWNSAMPLE_RATIO) ** 2, HIDDEN_SIZE),
            nn.GELU(),
    ...
    def forward(self, pixel_values):
        vit_embeds = vit(pixel_values, False, True).last_hidden_state
        vit_embeds = vit_embeds[:, 1:, :]

        h = w = int(vit_embeds.shape[1] ** 0.5)
        vit_embeds = vit_embeds.reshape(vit_embeds.shape[0], h, w, -1)
        vit_embeds = self.pixel_shuffle(vit_embeds, scale_factor=DOWNSAMPLE_RATIO)
...
def convert_vision_transformer():
    ...
    pixel_values = torch.randn((13, CHANNELS, IMAGE_SIZE, IMAGE_SIZE))
...
def convert_block_cache(layer_id):
    ...
    past_k = torch.randn((1, SEQ_LENGTH, config.llm_config.num_key_value_heads, HEAD_DIM))
    past_v = torch.randn((1, SEQ_LENGTH, config.llm_config.num_key_value_heads, HEAD_DIM))
...
if not os.path.exists(folder):
    os.makedirs(folder)
# create vit folder to store onnx
