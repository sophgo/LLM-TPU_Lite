...
public:
  ...
  void deinit();
  int forward_first(std::vector<int> &tokens, std::vector<float> &pixel_values, int begin, int offset);
  int forward_next();
  std::vector<int> generate(std::vector<int> &history_tokens, std::vector<float> &pixel_values, int begin, int offset, int EOS);
...
void Phi3::net_launch(const bm_net_info_t *net, int stage_idx) {
...
void Phi3::init(const std::vector<int> &devices, std::string model_path) {
...
void Phi3::head_launch(const bm_net_info_t *net, bm_device_mem_t &logits_mem) {
  std::vector<bm_tensor_t> in_tensors(net->input_num);
  std::vector<bm_tensor_t> out_tensors(net->output_num);
...
int Phi3::penalty_sample(const bm_net_info_t *net, bm_device_mem_t &logits_mem) {
...
int Phi3::forward_first(std::vector<int> &tokens, std::vector<float> &pixel_values, int begin, int offset) {
...
std::vector<int> Phi3::generate(std::vector<int> &history_tokens, std::vector<float> &pixel_values, int begin, int offset, int EOS) {
  ...
  int token = forward_first(history_tokens, pixel_values, begin, offset);
...
PYBIND11_MODULE(chat, m) {
    pybind11::class_<Phi3>(m, "Phi3")
        .def(pybind11::init<>())
        .def("init", &Phi3::init)
        .def("forward_first", &Phi3::forward_first)
        .def("forward_next", &Phi3::forward_next)
        .def("generate", &Phi3::generate)
        .def("deinit", &Phi3::deinit)
        .def_readwrite("SEQLEN", &Phi3::SEQLEN) // read SEQLEN in pipeline.py
        .def_readwrite("token_length", &Phi3::token_length)
        .def_readwrite("temperature", &Phi3::temperature)
        .def_readwrite("top_p", &Phi3::top_p)
        .def_readwrite("repeat_penalty", &Phi3::repeat_penalty)
        .def_readwrite("repeat_last_n", &Phi3::repeat_last_n)
        .def_readwrite("max_new_tokens", &Phi3::max_new_tokens)
        .def_readwrite("generation_mode", &Phi3::generation_mode)
        .def_readwrite("prompt_mode", &Phi3::prompt_mode);
}
