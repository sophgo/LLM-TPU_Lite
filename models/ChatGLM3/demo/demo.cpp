...
void ChatGLM::init(std::string model_path, std::string tokenizer_path) {
  ...
  for (int idx = 0; idx < NUM_LAYERS; idx++) {
    ...
    if (idx == 0) {
      ...
      bm_memcpy_s2d(bm_handle, in2_mem, (void *)attention_mask.data());
        ...
        d2d(in1_mem, net_blocks[0]->stages[0].input_mems[1]);
        d2d(in2_mem, net_blocks[0]->stages[0].input_mems[2]);
    }
...
int main(int argc, char **argv) {
  ...
  printf("Demo for ChatGLM in BM1688\n");
  ...
  glm.init(model_path, tokenizer_path);
  printf("==========================\n");
  glm.chat();
  glm.deinit();
  return 0;
