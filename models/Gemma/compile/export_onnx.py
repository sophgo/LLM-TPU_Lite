...
class GemmaBlock(torch.nn.Module):
    ...
    def __init__(self, layer_id):
        ...
        self.cos_emb = self.rotary[0].view(SEQ_LENGTH, HEAD_DIM).bfloat16().to("cuda")
        self.sin_emb = self.rotary[1].view(SEQ_LENGTH, HEAD_DIM).bfloat16().to("cuda")
...
def convert_block(layer_id):
    ...
    hidden_states = torch.randn((1, SEQ_LENGTH, HIDDEN_SIZE)).bfloat16().to("cuda")
    position_ids = torch.tensor([range(SEQ_LENGTH)], dtype=torch.long).to("cuda")
    attention_mask = torch.randn((1, 1, SEQ_LENGTH, SEQ_LENGTH)).bfloat16().to("cuda")

    torch.onnx.export(model, (hidden_states, position_ids, attention_mask),
...
def convert_block_cache(layer_id):
    ...
    attention_mask = torch.ones((1, 1, 1, SEQ_LENGTH + 1)).bfloat16().to("cuda")
    past_k = torch.randn((1, 1, SEQ_LENGTH, HEAD_DIM)).bfloat16().to("cuda")
    past_v = torch.randn((1, 1, SEQ_LENGTH, HEAD_DIM)).bfloat16().to("cuda")
...
def test_net_with_mask():
    ...
    attention_mask = attention_mask.view(1, 1, SEQ_LENGTH, SEQ_LENGTH).to("cuda")
    k_cache = []
    v_cache = []
    for i in range(NUM_LAYERS):
        out, k, v = blocks[i](out.bfloat16(), position_ids, attention_mask.bfloat16())
        k[:, :, token_len:, :] = 0
        v[:, :, token_len:, :] = 0
        k_cache.append(k)
        v_cache.append(v)
    ...
    while int(token) != tokenizer.eos_token_id and token_len < SEQ_LENGTH:
        ...
        attention_mask = torch.zeros((1, 1, 1, SEQ_LENGTH + 1)).float().to("cuda")
        attention_mask[:, :, :, token_len - 1:SEQ_LENGTH] = -10000.0
        for i in range(NUM_LAYERS):
            out, k, v = block_kvs[i](out.bfloat16(), position_ids, attention_mask.bfloat16(),
